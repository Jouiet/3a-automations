...
function doGet(e) {
  // Support both simple health check and full API via GET
  const params = e.parameter || {};

  // If no action, return health check
  if (!params.action) {
    return ContentService
  ...
  // Process action via GET (same as POST)
  const response = { success: false };
...
function getDashboardStats() {
  ...
  const thisMonthMetrics = metrics.filter(m => {
  ...
  const revenueThisMonth = thisMonthMetrics.reduce((sum, m) => sum + (parseFloat(m.value) || 0), 0);
  const revenueLastMonth = lastMonthMetrics.reduce((sum, m) => sum + (parseFloat(m.value) || 0), 0);

  // Calculate growth percentage (0 if no previous month data)
