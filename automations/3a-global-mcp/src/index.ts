...
async function executeScript(scriptPath: string, args: string[] = [], options: any = {}): Promise<any> {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "run_email_automation": {
            ...
            if (json_data) emailArgs.push(`--json=${json_data}`);
        ...
        case "run_linkedin_pipeline": {
            ...
            const linkArgs = [];
            if (search_query) linkArgs.push(`--search=${search_query}`);
            else if (company_url) linkArgs.push(`--company=${company_url}`);
        ...
        case "run_bigbuy_sync": {
            ...
            const bbArgs = [];
            if (action === "health") bbArgs.push("--health");
            else if (action === "list_categories") bbArgs.push("--categories");
            else if (action === "list_products") bbArgs.push("--products", `--category=${category_id}`, `--limit=${limit}`);
            else if (action === "sync_category") bbArgs.push("--sync", `--category=${category_id}`, `--limit=${limit}`);
        ...
        case "run_whatsapp_booking": {
            ...
            const wArgs = [`--${wAction}`];
            if (wParam) wArgs.push(`--phone=${wParam}`);
            if (wName) wArgs.push(`--name=${wName}`);
            if (wDate) wArgs.push(`--date=${wDate}`);
            if (wTime) wArgs.push(`--time=${wTime}`);
            if (wService) wArgs.push(`--service=${wService}`);
