...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "generate_blog_post") {
        const { topic, language = "fr", keywords, publish, distribute } = args as any;
        ...
        if (keywords) scriptArgs.push(`--keywords=${keywords}`);
        if (publish) scriptArgs.push("--publish");
        if (distribute) scriptArgs.push("--distribute");
    ...
    if (name === "run_email_automation") {
        const { mode, email, json_data } = args as any;
        const scriptArgs = [
            "/Users/mac/Documents/JO-AAA/automations/agency/email-automation-unified.cjs",
            `--mode=${mode}`,
            `--email=${email}`
        ];
        if (json_data) scriptArgs.push(`--json=${json_data}`);
    ...
    if (name === "run_linkedin_pipeline") {
        const { search_query, company_url, max_results = 50 } = args as any;
        const scriptArgs = [
            "/Users/mac/Documents/JO-AAA/automations/agency/linkedin-lead-automation.cjs"
        ];

        if (search_query) scriptArgs.push(`--search=${search_query}`);
        else if (company_url) scriptArgs.push(`--company=${company_url}`);
        else throw new Error("Either search_query or company_url is required");
        ...
        return new Promise((resolve, reject) => {
            ...
            ps.on("close", (code: number) => {
                ...
                if (code === 0) resolve({ content: [{ type: "text", text: output }] });
                else resolve({ isError: true, content: [{ type: "text", text: `Pipeline Finished with Status ${code}:\n${output}\nErrors:\n${errorOutput}` }] });
            });
        });
    }

    if (name === "run_bigbuy_sync") {
        const { action, category_id, limit = 20 } = args as any;
        ...
        else if (action === "list_categories") scriptArgs.push("--categories");
        else if (action === "list_products") {
            if (!category_id) throw new Error("category_id is required for list_products");
            scriptArgs.push("--products", `--category=${category_id}`, `--limit=${limit}`);
        }
        else if (action === "sync_category") {
            if (!category_id) throw new Error("category_id is required for sync_category");
    ...
    if (name === "run_google_maps_pipeline") {
        const { query, location, max_results = 50, test_mode } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/google-maps-to-klaviyo-pipeline.cjs"];

        if (test_mode) {
            scriptArgs.push("--test");
        } else {
            if (!query || !location) throw new Error("query and location are required unless test_mode is true");
    ...
    if (name === "run_newsletter_automation") {
        const { topic, language = "fr", action } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/newsletter-automation.cjs"];

        if (action === "test") {
            scriptArgs.push("--test");
        } else if (action === "preview") {
            if (!topic) throw new Error("topic is required for preview");
            scriptArgs.push("--preview", `--topic=${topic}`, `--lang=${language}`);
        } else if (action === "send") {
            if (!topic) throw new Error("topic is required for send");
    ...
    if (name === "run_voice_api") {
        const { action, input, email } = args as any;
        ...
        if (action === "test") {
            ...
            if (!email) throw new Error("email is required for qualify");
    ...
    if (name === "check_telephony_bridge") {
        ...
        if (action === "health") scriptArgs.push("--health");
        else if (action === "test_grok") scriptArgs.push("--test-grok");
    ...
    if (name === "run_chatbot_qualifier") {
        const { action, input, session_id } = args as any;
        ...
        if (action === "health") scriptArgs.push("--health");
            ...
            if (!session_id) throw new Error("session_id is required for qualify");
    ...
    if (name === "run_churn_prediction") {
        const { action, email, daysSinceLastPurchase, totalOrders, totalSpent } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/churn-prediction-resilient.cjs"];

        if (action === "health") scriptArgs.push("--health");
        else if (action === "predict") {
            if (!email) throw new Error("email is required for predict");
    ...
    if (name === "run_review_automation") {
        ...
        if (action === "health") scriptArgs.push("--health");
        else if (action === "test") scriptArgs.push("--test");
        else if (action === "process_pending") scriptArgs.push("--process-pending");
    ...
    if (name === "manage_prompt_tracker") {
        const { command, prompt_id, category, tool, scores, notes } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/prompt-feedback-tracker.cjs", command];

        if (prompt_id) scriptArgs.push(prompt_id);
        if (command === "add") {
            if (category) scriptArgs.push(category);
            if (tool) scriptArgs.push(tool);
        }
        if (command === "score") {
            if (scores) {
            ...
            if (notes) scriptArgs.push(notes);
        }
    ...
    if (name === "run_product_photo_generator") {
        const { action, image, prompt, analyze_image } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/product-photos-resilient.cjs"];

        if (action === "health") scriptArgs.push("--health");
        else if (action === "enhance") {
            if (!image || !prompt) throw new Error("image and prompt required for enhance");
            scriptArgs.push(`--image=${image}`, `--prompt=${prompt}`);
        } else if (action === "analyze") {
            if (!analyze_image || !prompt) throw new Error("analyze_image and prompt required for analyze");
    ...
    if (name === "run_price_drop_monitor") {
        const { action, product_id, drop_percent, email } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/price-drop-alerts.cjs"];

        if (action === "health") scriptArgs.push("--health");
        else if (action === "check") scriptArgs.push("--check-prices");
        else if (action === "simulate") {
            scriptArgs.push("--simulate");
            if (product_id) scriptArgs.push(`--product-id=${product_id}`);
            if (drop_percent) scriptArgs.push(`--drop=${drop_percent}`);
            if (email) scriptArgs.push(`--email=${email}`);
        } else if (action === "server") {
            scriptArgs.push("--server");
        }
    ...
    if (name === "run_cjdropshipping_api") {
        const { action, tracking_number } = args as any;
        ...
        if (action === "health") { /* default behavior is health check */ }
            ...
            if (!tracking_number) throw new Error("tracking_number required for track");
    ...
    if (name === "manage_dropshipping_orders") {
        const { command, order_id } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/dropshipping-order-flow.cjs"];

        if (command === "health") scriptArgs.push("--health");
        else if (command === "sync_tracking") scriptArgs.push("--sync-tracking");
        else if (command === "list_orders") scriptArgs.push("--list-orders");
        else if (command === "get_order") {
            if (!order_id) throw new Error("order_id required for get_order");
    ...
    if (name === "run_birthday_anniversary_flow") {
        const { action, email, type } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/birthday-anniversary-flow.cjs"];

        if (action === "health") scriptArgs.push("--health");
        else if (action === "scan") scriptArgs.push("--scan-upcoming");
        else if (action === "test") {
            scriptArgs.push("--test");
            if (email) scriptArgs.push(`--email=${email}`);
            if (type) scriptArgs.push(`--type=${type}`);
        } else if (action === "server") {
            scriptArgs.push("--server");
        }
    ...
    if (name === "manage_referral_program") {
        const { command, customer_id, code, new_customer_email } = args as any;
        ...
        if (command === "health") scriptArgs.push("--health");
            ...
            if (customer_id) scriptArgs.push(`--customer-id=${customer_id}`);
        } else if (command === "process") {
            scriptArgs.push("--process-referral");
            if (code) scriptArgs.push(`--code=${code}`);
            if (new_customer_email) scriptArgs.push(`--new-customer=${new_customer_email}`);
        } else if (command === "leaderboard") scriptArgs.push("--leaderboard");
        else if (command === "stats") scriptArgs.push("--stats");
        else if (command === "server") scriptArgs.push("--server");
    ...
    if (name === "run_replenishment_reminder") {
        const { action, customer_id, email } = args as any;
    ...
    if (name === "run_sms_automation") {
        const { action, phone } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/sms-automation-resilient.cjs"];

        if (action === "health") scriptArgs.push("--health");
        else if (action === "test_cart") scriptArgs.push("--test-abandoned-cart");
        else if (action === "test_order") scriptArgs.push("--test-order");
        else if (action === "test_shipping") scriptArgs.push("--test-shipping");
        else if (action === "subscribe") {
            scriptArgs.push("--subscribe");
            if (phone) scriptArgs.push(`--phone=${phone}`);
        }
    ...
    if (name === "run_at_risk_customer_flow") {
        const { action, customer } = args as any;
        ...
        if (action === "health") scriptArgs.push("--health");
            ...
            if (customer) scriptArgs.push(`--customer=${customer}`);
        } else if (action === "server") {
            scriptArgs.push("--server");
        }
    ...
    if (name === "manage_lead_gen_scheduler") {
        const { command, tier, market, query, location, topic, dry_run } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/lead-gen-scheduler.cjs"];

        if (command === "status") scriptArgs.push("--status");
        else {
            // Map command to pipeline arg
            if (command === "daily") scriptArgs.push("--pipeline=daily");
            else if (command === "tier1") scriptArgs.push("--pipeline=tier1");
            else if (command === "linkedin") scriptArgs.push("--pipeline=linkedin");
            else if (command === "gmaps") scriptArgs.push("--pipeline=gmaps");
            else if (command === "newsletter") scriptArgs.push("--pipeline=newsletter");

            if (tier) scriptArgs.push(`--tier=${tier}`);
            if (market) scriptArgs.push(`--market=${market}`);
            if (query) scriptArgs.push(`--query=${query}`);
            if (location) scriptArgs.push(`--location=${location}`);
            if (topic) scriptArgs.push(`--topic=${topic}`);
    ...
    if (name === "manage_voice_widget_templates") {
        const { command, preset, client, domain, deploy } = args as any;
        const scriptArgs = ["/Users/mac/Documents/JO-AAA/automations/agency/core/voice-widget-templates.cjs"];

        if (command === "list") scriptArgs.push("--list");
        else if (command === "generate") {
            if (preset) scriptArgs.push(`--preset=${preset}`);
            if (client) scriptArgs.push(`--client=${client}`);
            if (domain) scriptArgs.push(`--domain=${domain}`);
            if (deploy) scriptArgs.push("--deploy");
    ...
    if (name === "run_email_personalization") {
        const { action, lead, context, cart } = args as any;
        ...
        if (action === "health") scriptArgs.push("--health");
            ...
            if (lead) scriptArgs.push(`--lead=${lead}`);
            ...
            if (cart) scriptArgs.push(`--cart=${cart}`);
        } else if (action === "server") {
            scriptArgs.push("--server");
        }
